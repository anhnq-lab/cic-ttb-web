...
const ProjectManager: React.FC<ProjectManagerProps> = ({ projects, form, setForm, editingId, setEditingId, onSubmit, onEdit, onDelete }) => {
    ...
    // Filter projects based on tab (defensive: ensure projects is an array)
    ...
    const handleSubmitWithValidation = (e: React.FormEvent) => {
        e.preventDefault();
        console.log('[ProjectManager] handleSubmitWithValidation called');
        console.log('[ProjectManager] Form data:', form);

        try {
            // Validate form data
            const validatedForm = validateProjectForm(form);
            console.log('[ProjectManager] Validation passed, calling onSubmit');

            // Call original onSubmit - pass the original event which already had preventDefault called
            // Don't create a fake event - just call onSubmit directly
            onSubmit(e);
        } catch (error) {
            console.error('[ProjectManager] Validation error:', error);
                ...
                <div className="bg-white p-6 rounded-xl shadow border border-gray-200 sticky top-4">
                    ...
                    <form onSubmit={handleSubmitWithValidation} className="space-y-4">
                        ...
                        <div>
                            ...
                            <select className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={form.service_type} onChange={e => setForm({ ...form, service_type: e.target.value as ProjectForm['service_type'] })}>
                        ...
                        <div>
                            ...
                            <select className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" value={form.status || 'published'} onChange={e => setForm({ ...form, status: e.target.value as ProjectForm['status'] })}>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                {[0, 1, 2, 3].map((index) => (
                                    <div key={index} className="relative">
                                        <span className="absolute -top-1 -left-1 z-10 bg-blue-600 text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-bold shadow">{index + 1}</span>
                                        <ImageUploader
            ...
            <div className="xl:col-span-2">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                        ...
                        >
                            Đã đăng ({safeProjects.filter(p => !p.status || p.status === 'published' || p.status === 'active').length})
                        </button>
                        <button
                        ...
                        >
                            Chờ duyệt ({safeProjects.filter(p => p.status === 'pending').length})
                        </button>
                    </div>
                </div>
